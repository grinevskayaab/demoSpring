...
//    Optional<Author> getAuthorTopByPlays();
    ...
    @Query(value= """
            select a from Author a join a.songs join a.songs.stat stat
            group by a.id
...
//    List<AuthorWithCash> getAuthorsCash();
    ...
    @Query(value= """
             with table1 as (
                 select sum(stat1.countPlays) as sum
                                     from Author a1 join a1.songs join a1.songs.stat stat1
                                     where stat1.date >= :date
             ), table2 as (
             select a.id as id, a.name as name,
             cast(
                round(
                    sum(stat.countPlays)*100/cast((select sum from table1) as float)
                ,2) * 1000000/100
              as Long) as cash
             from Author a join a.songs join a.songs.stat stat
             where stat.date > :date
             GROUP BY a.id
             )
            select new com.github.grinevskayaab.demo.dto.AuthorWithCash(id, name, cash)
             from table2""")
    List<AuthorWithCash> getAuthorsCash(Timestamp date);
}
